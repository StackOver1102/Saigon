...
const EditProductMain = (props) => {
  ...
  const handleUpdate = async (e) => {
    ...
    await mutation.mutate({
      ...
      description,
      content: textContent,
      image: uploadedImageUrls.length > 0 ? updateProductSingle : urlList,
    });
  };
  ...
  const onEditorStateChange = (newEditorState) => {
    ...
    setTextContent(draftToHtml(convertToRaw(newEditorState.getCurrentContent())));
  };

  useEffect(() => {
    if (dataDetail) {
      setDescription(dataDetail.description);
      setUrlList(dataDetail.image);
      setTittle(dataDetail.tittle);
      const blocksFromHtml = htmlToDraft(dataDetail.content);
      ...
      <section className="content-main" style={{ maxWidth: "1200px" }}>
        ...
        <form onSubmit={handleUpdate}>
          <div className="content-header">
            <Link to="/events" className="btn btn-danger text-white">
              Go to list events
            </Link>
            <h2 className="content-title">Edit Event</h2>
            <div>
